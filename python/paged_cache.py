...
class PagedPool:
    ...
    def __init__(self, *, num_layers, num_kv_heads, head_dim, max_blocks,
        ...
        assert self.manager.create_sequence(self.sequence_id, self.sequence_id)
        ...
        self.spacer_id = self.sequence_id + 1000
        if scatter:
            assert self.manager.create_sequence(self.spacer_id, self.spacer_id)

    def ensure_capacity(self, total_tokens):
        """Grows the block table to cover `total_tokens`, if it does not already.
        ...
        """
        have = len(self.block_table())
        need = -(-total_tokens // self.tokens_per_block)
        for _ in range(need - have):
            assert self.manager.reserve_tokens(self.sequence_id, self.tokens_per_block), (
...
class PagedLayer(CacheLayerMixin):
    ...
    def update(self, key_states, value_states, *args, **kwargs):
        ...
        while position < end:
            ...
            frame = self.pool.writable_frame(position)
